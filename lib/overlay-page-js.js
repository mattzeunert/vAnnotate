function initializeAnnotations(results){
    ...
    function showAnnotation(annotationIndex){
    ...
    var cursorIsInsideAnnotationBubble = false;
    annotationElement.on('mouseenter', function(){
        cursorIsInsideAnnotationBubble = true;
    })
    annotationElement.on('mouseleave', function(){
        cursorIsInsideAnnotationBubble = false;
    ...
    $('.annotated-element').on('mouseleave', function(){
        ...
        setTimeout(function(){
            if (cursorIsInsideAnnotationBubble){return;}
            if (currentAnnotationIndex === annotationIndex){
                annotationElement.hide();
            }
        }, 100)
    });
