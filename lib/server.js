...
var getFileResults = function(results, code, filename){
    return {
        setup: logInjector.getLoggingSetup(code),
        results: results,
        fileHash: helper.sha1(code)
    }
}
            ...
            if (exists){
                serverFs.readFile(filePath, function(err, code){
                    ...
                    if (results !== undefined){
                        ...
                        if (showJsonResults){
                            res.end(JSON.stringify(getFileResults(results, code, filename)))
                        } else {
...
exports.start = function(options){
    ...
    log('Listening on port ' + options.port, 'always');
    return server;
}
