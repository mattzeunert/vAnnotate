...
var getFileResults = function(results, code, filePath){
    return {
...
var generateOverlayHtml = function(results, code, filename){
    ...
    codeOverlay.generate(loggingSetup, results, code).then(function(html){
        var path = require('path');
        overlayHtml = fs.readFileSync(path.resolve(__dirname, './overlay-page.html')).toString();
        overlayHtml = overlayHtml.replace('{{code}}', html);

        var overlayCss = fs.readFileSync(path.resolve(__dirname, './pygments-javascript.css')).toString();
        overlayHtml = overlayHtml.replace('{{overlayPageCss}}', overlayCss);

        overlayHtml = overlayHtml.replace('{{bookmarkletCode}}', fs.readFileSync(path.resolve(__dirname, '../bookmarklet.js')).toString());

        overlayHtml = overlayHtml.replace('{{filename}}', filename);
        def.resolve(overlayHtml);
    });
    ...
    if (saveResults){
        if (filePath === undefined){
            ...
            serverFs.readFile('.' + filePath, function(err, data){
                ...
                var fileResults = getFileResults(results, code, filePath);
            ...
            if (exists){
                serverFs.readFile(filePath, function(err, code){
                    ...
                    if (results !== undefined){
                        ...
                        if (showJsonResults){
                            res.end(JSON.stringify(getFileResults(results, code, filePath)))
                        } else {
                            generateOverlayHtml(results, code, filename).then(function(html){
                                res.end(html);
                            })
                        }
                    }
