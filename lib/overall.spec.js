...
function createAnnotations(){	
    ...
    var writeFile = Promise.convertNodeAsyncFunction( fs.writeFile );
	...
	mkdirp(siteDirectory, function(){
		...
		expectedAnnotations.forEach(function(annotation, i){
			var filename = i + '.js';
			var promise = writeFile(siteDirectory + filename, annotation.code);
			promises.push(promise);
			html += '<script src="' + filename + '"></script>';
		})

		promises.push(writeFile(siteDirectory + 'tests.html', html));
	...
	waitsFor(function(){
		return done;
	}, 'Didn\'t complete annotations in time.', 3000)
