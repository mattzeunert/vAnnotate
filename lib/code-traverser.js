...
var traverseParseTree = function(treeElement, handlers){
    var isArray = treeElement.length !== undefined,
        ...
        recurse = function(element){
            element.__debug_parent = treeElement;
            traverseParseTree(element, handlers)
        },
        evaluatePrependToBody = function(modify, body){
            if (modify && modify.prependToBody){
                modify.prependToBody.forEach(function(instruction){
                    body.unshift(instruction);
                });
            }
        };

    // have it in scope for debugging
    escodegen
    ...
    handlers.forEach(function(handler){
        ...
        if (conditions.isFunction(treeElement)){
            evaluatePrependToBody(treeElement.modify, treeElement.body.body)
        }
    });
