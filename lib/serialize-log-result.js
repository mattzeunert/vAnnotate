...
function escapeLineBreaks(str){
    return str.replace(/([\n\u2028\u2029])/g, function(val){
        return convertToEscapedUnicode(val);
    });
}

function convertToEscapedUnicode(str){
...
var serializeLogResult = function(logResult, preventDiggingDeeper, reallyPreventGoingDeeper){
    ...
    if (type === 'string'){
        ...
        return '"' + escapeLineBreaks(logResult) + '"';
    ...
    if (!isArray && type === 'object'){
        ...
        for (property in logResult){
            ...
            serialized += '   ' + (serializedPropertyCount !== 0 ? ' ' : '') + escapeLineBreaks(property) + ': ' + valueString + '\n';
